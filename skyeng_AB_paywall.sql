--1210 solutions.Monetization.LimitedOneDaySubscription.user
...
max(case when value::varchar ='"test"' then 'A' 
...
where type in ('assistant_buy_access_clicked','assistant_buy_access_shown', 'assistant_solution_loaded') and payload->>'mobileApp'='false' and created_at between '2022-11-23' and '2022-12-14'
group by first_test_d, groups)

-- select
--     groups,
--     -- sum(active_users) as active_us,
--     sum(paywall_show) as paywall_sh,
--     -- sum(paywall_show)/sum(active_users) CR_pwsh,
--     sum(paywall_click) as paywall_cl
--     -- sum(paywall_click)/sum(paywall_show) CR_pwcl,
--     -- sum(users_test) as users_tst
-- from (
